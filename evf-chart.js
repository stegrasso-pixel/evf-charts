/*!
 * EVF Charts of the Month | chart engine | version 1.8.0 | 23 September 2026
...
function fmt2(v, dec) {
  if (v === null || v === undefined || isNaN(v)) return '';
  const f = C.y2Axis || {}; const d = dec !== undefined ? dec : (f.decimals !== undefined ? f.decimals : 0);
  const a = Math.abs(v).toLocaleString('en-GB', { minimumFractionDigits: d, maximumFractionDigits: d });
  return (v < 0 ? MINUS : '') + (f.prefix || '') + a + (f.suffix || '');
}
function axis2() {
  const y = C.y2Axis || {};
  const sc = { type: 'linear', position: 'right', beginAtZero: y.zeroBased === true, grid: { display: false },
    border: { display: false }, ticks: { padding: px(14), maxTicksLimit: 6, color: K.muted,
      callback: v => fmt2(v, y.tickDecimals !== undefined ? y.tickDecimals : 0) }, title: axisTitle(y) };
  if (y.min !== null && y.min !== undefined) sc.min = y.min;
  if (y.max !== null && y.max !== undefined) sc.max = y.max;
  return sc;
}
function isBarSeries(s) { return (s.chart || 'bar') === 'bar'; }
function isPie() { return T === 'pie' || T === 'donut'; }
const PIE_REST = ['rgba(255,255,255,0.85)', '#5C768C', 'rgba(255,255,255,0.40)', '#3E5A73', '#707070'];
function pieVals() { const s = (C.series || [])[0] || {}; return (s.data || []).map(v => (typeof v === 'number' && v > 0) ? v : 0); }
function sliceColours(L) {
  const hl = (C.highlight || []).length > 0; const isOther = l => /^other\b/i.test(String(l));
  const rest = L.some(isOther) ? PIE_REST.filter(c => c !== '#707070') : PIE_REST.slice(); let r = 0;
  return L.map((lab, i) => { if (hl ? isHi(lab, i) : i === 0) return '#1DB4D7'; if (isOther(lab)) return '#707070'; return rest[r++ % rest.length]; });
}
function shares(vals) {
  const tot = vals.reduce((a, b) => a + b, 0); if (!tot) return vals.map(() => 0);
  const raw = vals.map(v => v / tot * 100); const fl = raw.map(Math.floor); let left = 100 - fl.reduce((a, b) => a + b, 0);
  raw.map((r, i) => [r - fl[i], i]).sort((a, b) => b[0] - a[0]).forEach(x => { if (left > 0) { fl[x[1]]++; left--; } });
  return fl;
}
function pieRows() {
  const L = C.labels || []; const vals = pieVals(); const pc = shares(vals); const cols = sliceColours(L);
  const showV = (C.valueLabels || 'auto') !== 'none';
  return L.map((n, i) => ({ n: String(n), v: showV ? fmt(vals[i]) : '', p: pc[i] + '%', c: cols[i] }));
}
function wrapText(c, t, max) {
  const words = String(t).split(/\s+/); const out = []; let cur = '';
  words.forEach(wd => { const n = cur ? cur + ' ' + wd : wd; if (c.measureText(n).width > max && cur) { out.push(cur); cur = wd; } else cur = n; });
  if (cur) out.push(cur); return out;
}
function pieKey(rows) {
  const c = document.createElement('canvas').getContext('2d'); let pw = 0, tw = 0; const cap = px(260);
  const lay = rows.map(r => { c.font = '800 ' + px(46) + "px 'Barlow'"; pw = Math.max(pw, c.measureText(r.p).width);
    c.font = '500 ' + px(24) + "px 'Inter Tight'"; const lines = wrapText(c, r.n, cap).slice(0, 3);
    lines.forEach(l => { tw = Math.max(tw, c.measureText(l).width); });
    c.font = '400 ' + px(22) + "px 'Inter Tight'"; tw = Math.max(tw, c.measureText(r.v).width);
    const hb = lines.length * px(30) + (r.v ? px(28) : 0);
    return { lines: lines, hb: hb, h: Math.max(px(52), hb) }; });
  pw = Math.ceil(pw) + px(6);
  return { pw: pw, lay: lay, w: Math.ceil(px(20) + px(16) + pw + px(16) + tw + px(12)), gap: px(48) };
}
...
function valueScale() {
  ...
  if ((T === 'bar' || T === 'hbar' || T === 'combo') && !y.log) {
    sc.bounds = 'data'; sc.ticks.includeBounds = false;
    sc.afterDataLimits = s => {
      const vals = [].concat(...(C.series || []).filter(x => T !== 'combo' || isBarSeries(x)).map(x => (x.data || []).filter(v => typeof v === 'number')), (C.refLines || []).map(r => r.value));
...
function build() {
  ...
  if (T === 'combo') {
    legendHTML(S.map(s => ({ n: s.name, c: isBarSeries(s) ? FILL[role(s)] : LINE[role(s)] })));
    const ds = S.map(s => isBarSeries(s)
      ? { type: 'bar', label: s.name, data: s.data, yAxisID: 'y', order: 2, borderWidth: 0,
          backgroundColor: c => { const hl = (C.highlight || []).length > 0;
            if (hl && !isHi(L[c.dataIndex], c.dataIndex)) return K.steel;
            if (isPart(c.dataIndex)) return K.partBar;
            return FILL[role(s)]; },
          categoryPercentage: 0.78, barPercentage: 0.86 }
      : { type: 'line', label: s.name, data: s.data, yAxisID: 'y1', order: 1, borderColor: LINE[role(s)],
          borderWidth: Math.max(3, px(5)), pointRadius: 0, spanGaps: false,
          segment: { borderDash: c => isPart(c.p1DataIndex) ? [px(14), px(10)] : undefined } });
    return { type: 'bar', data: { labels: L, datasets: ds },
      options: { layout: { padding: { top: px(52) } }, scales: { x: catScale(false), y: valueScale(), y1: axis2() } } };
  }
  if (T === 'stacked') {
  ...
  if (isPie()) {
    const s = S[0] || { data: [] }; const rows = pieRows(); const k = pieKey(rows);
    const vp = px(12);
    const pad = c => { const cw = c.chart.width, ch = c.chart.height;
      const d = Math.max(px(220), Math.min(ch - 2 * vp, cw - k.w - k.gap));
      const left = Math.max(0, Math.round((cw - (d + k.gap + k.w)) / 2));
      return { top: vp, bottom: vp, left: left, right: Math.max(0, cw - left - d) }; };
    return { type: 'doughnut', data: { labels: L, datasets: [{ label: s.name || '', data: pieVals(), backgroundColor: rows.map(r => r.c),
        borderColor: K.navy, borderWidth: Math.max(3, px(5)), hoverOffset: 0 }] },
      options: { cutout: T === 'donut' ? '58%' : 0, layout: { padding: pad } } };
  }
  throw new Error('unknown type "' + T + '". Use line, bar, hbar, stacked, combo, scatter, pie, donut or custom.');
}
function endSpace() {
...
const evfPlugin = { id: 'evf', afterDatasetsDraw(chart) {
  ...
  if (T === 'combo') {
    S.forEach((s, k) => { if (isBarSeries(s)) return; const i = lastIdx(s.data); if (i < 0) return;
      const pt = chart.getDatasetMeta(k).data[i]; if (!pt) return;
      ctx.font = '700 ' + px(28) + "px 'Barlow'"; ctx.fillStyle = LINE[role(s)];
      const slot = (A.right - A.left) / Math.max(1, (C.labels || []).length);
      ctx.textAlign = 'right'; ctx.textBaseline = 'bottom';
      ctx.fillText(fmt2(s.data[i]), pt.x - Math.max(px(10), slot), pt.y - px(14)); });
  }
  let lblS = null, lblK = 0;
  if ((T === 'bar' || T === 'hbar') && S.length === 1) { lblS = S[0]; lblK = 0; }
  if (T === 'combo' && S.filter(isBarSeries).length === 1) { lblK = S.findIndex(isBarSeries); lblS = S[lblK]; }
  if (lblS) {
    const s = lblS; const meta = chart.getDatasetMeta(lblK); const n = s.data.length; const hb = T === 'hbar';
    ...
    meta.data.forEach((bar, i) => { const v = s.data[i]; if (v === null || v === undefined) return;
      ...
      else { const t = fmt(v); const tw = ctx.measureText(t).width; const x = Math.max(A.left + tw / 2, Math.min(bar.x, chart.width - tw / 2 - px(4)));
  ...
  if (isPie()) {
    const a0 = chart.getDatasetMeta(0).data[0];
    if (a0) {
      const cx = a0.x, cy = a0.y, ro = a0.outerRadius, ri = a0.innerRadius; const rows = pieRows(); const k = pieKey(rows);
      const gap = px(26); const tot = k.lay.reduce((a, l) => a + l.h, 0) + (rows.length - 1) * gap;
      const x0 = Math.min(cx + ro + k.gap, chart.width - k.w); let y = Math.max(px(4), cy - tot / 2);
      rows.forEach((r, i) => { const l = k.lay[i]; const mid = y + l.h / 2;
        ctx.fillStyle = r.c; ctx.fillRect(x0, mid - px(10), px(20), px(20));
        ctx.font = '800 ' + px(46) + "px 'Barlow'"; ctx.fillStyle = r.c === K.cyan ? K.cyan : K.white;
        ctx.textAlign = 'left'; ctx.textBaseline = 'middle'; ctx.fillText(r.p, x0 + px(36), mid + px(2));
        const tx = x0 + px(36) + k.pw + px(16); let ty = y + (l.h - l.hb) / 2;
        ctx.font = '500 ' + px(24) + "px 'Inter Tight'"; ctx.fillStyle = 'rgba(255,255,255,0.90)'; ctx.textBaseline = 'top';
        l.lines.forEach(t => { ctx.fillText(t, tx, ty + px(2)); ty += px(30); });
        if (r.v) { ctx.font = '400 ' + px(22) + "px 'Inter Tight'"; ctx.fillStyle = K.muted; ctx.fillText(r.v, tx, ty + px(2)); }
        y += l.h + gap; });
      if (T === 'donut' && ri > px(40)) {
        const ce = C.center || {}; const val = ce.value !== undefined ? String(ce.value) : fmt(pieVals().reduce((a, b) => a + b, 0));
        const lab = ce.label !== undefined ? String(ce.label) : ((C.series || [])[0] || {}).name || '';
        let fs = px(60); ctx.font = '800 ' + fs + "px 'Barlow'";
        while (ctx.measureText(val).width > ri * 1.6 && fs > px(30)) { fs -= 2; ctx.font = '800 ' + fs + "px 'Barlow'"; }
        ctx.font = '500 ' + px(22) + "px 'Inter Tight'"; const lines = lab ? wrapText(ctx, lab, ri * 1.45) : [];
        const showLab = lines.length > 0 && lines.length <= 2 && fs * 0.18 + px(14) + lines.length * px(28) <= ri * 0.8;
        ctx.font = '800 ' + fs + "px 'Barlow'"; ctx.fillStyle = K.white; ctx.textAlign = 'center';
        if (showLab) { ctx.textBaseline = 'alphabetic'; ctx.fillText(val, cx, cy + fs * 0.18);
          ctx.font = '500 ' + px(22) + "px 'Inter Tight'"; ctx.fillStyle = K.muted; ctx.textBaseline = 'top';
          lines.forEach((ln, j) => ctx.fillText(ln, cx, cy + fs * 0.18 + px(14) + j * px(28))); }
        else { ctx.textBaseline = 'middle'; ctx.fillText(val, cx, cy + px(2)); }
      }
    }
  }
  ctx.restore();
...
function lint(headOK) {
  ...
  if (T === 'custom') {
    ...
    if (T !== 'line' && T !== 'scatter' && !isPie() && (C.yAxis || {}).min > 0) w.push('Bars always start at zero. yAxis.min was ignored.');
  }
  ...
  if ((C.stats || []).length > 4) w.push('More than 4 stats. Keep 4 or fewer.');
  if (isPie()) {
    if (S.length !== 1) w.push('A pie chart takes one series. Put the slice names in labels and their values in data.');
    if (((S[0] || {}).data || []).some(v => typeof v !== 'number' || v <= 0)) w.push('Pie slices must be positive numbers.');
    if (L.length < 2 || L.length > 6) w.push('A pie chart needs 2 to 6 slices. Merge the smallest into "Other".');
    if ((C.refLines || []).length) w.push('Reference lines do not apply to a pie chart.');
  }
  if (T === 'combo') {
    const nb = S.filter(isBarSeries).length; const y2 = C.y2Axis || {};
    if (!nb || nb === S.length) w.push('A combo chart needs at least one bar series and one series with "chart": "line".');
    if (!y2.title && !y2.suffix && !y2.prefix) w.push('The right axis needs a unit. Set y2Axis.title or y2Axis.suffix.');
    if (!(C.yAxis || {}).title && !(C.numberFormat || {}).suffix) w.push('The left axis needs a unit. Set yAxis.title or numberFormat.suffix.');
  }
...
function render() {
  ...
  document.getElementById('stats').innerHTML = (C.stats || []).slice(0, 4).map(x =>
    '<div><div class="evf-stat-big">' + esc(x.value) + '</div><div class="evf-stat-cap">' + esc(x.label || '') + '</div></div>').join('');
...
function start() {
  ...
  if (!C) { const el = document.getElementById('evf-config'); if (el) { try { C = relaxedParse(el.textContent); } catch (e) {} } }
