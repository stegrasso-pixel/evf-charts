/*!
 * EVF Charts of the Month | chart engine | version 1.4.0 | 21 September 2026
...
let T = 'line', F = 1, W = 1080, H = 1350, CHART = null, CHECK = null, REMOVED = 0, LIBERR = null, ERR = null;
...
function tick(v) { const y = C.yAxis || {}; return fmt(v, y.tickDecimals !== undefined ? y.tickDecimals : 0, true); }
...
function build() {
  ...
  if (T === 'bar' || T === 'hbar') {
    ...
    const ds = S.map(s => ({ label: s.name, data: s.data,
      backgroundColor: c => { const v = s.data[c.dataIndex];
        if (single && v < 0) return K.negBar;
        if (single && isPart(c.dataIndex)) return K.partBar;
        if ((C.highlight || []).length) return isHi(L[c.dataIndex]) ? K.posBar : K.steel;
        return FILL[role(s)]; },
      borderColor: c => (single && s.data[c.dataIndex] < 0) ? K.negEdge : 'rgba(0,0,0,0)',
      borderWidth: c => (single && s.data[c.dataIndex] < 0) ? 2 : 0,
...
const evfPlugin = { id: 'evf', afterDatasetsDraw(chart) {
  ...
  if ((T === 'bar' || T === 'hbar') && S.length === 1) {
    ...
    const mode = C.valueLabels === 'auto' || !C.valueLabels ? (n <= 16 ? 'all' : 'last') : C.valueLabels;
    meta.data.forEach((bar, i) => { const v = s.data[i]; if (v === null || v === undefined) return;
      if (isPart(i)) { ctx.setLineDash([px(8), px(6)]); ctx.strokeStyle = K.cyan; ctx.lineWidth = 2;
        ...
        ctx.setLineDash([]); }
      if (mode === 'none' || (mode === 'last' && i !== lastIdx(s.data))) return;
      ...
      else { ctx.textAlign = 'center'; ctx.textBaseline = v < 0 ? 'top' : 'bottom'; ctx.fillText(fmt(v), bar.x, v < 0 ? bar.y + px(10) : bar.y - px(10)); }
    });
  }
  if (T === 'scatter') {
...
function render() {
  const d = dims(); T = C.type || 'line'; F = d.f; W = d.w; H = d.h; ERR = null; REMOVED = 0;
